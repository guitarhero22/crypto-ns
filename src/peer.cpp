...
std::vector<Event *> Peer::genTxn(Ticks genTime)
{
    ...
    Txn txn = Txn(this->ID, send_to, 50, genTime);
...
std::vector<Event *> Peer::genBlk(Ticks startTime, Ticks genTime, BID_t parent)
{
    ...
    Blk blk = Blk(forNxtBlk, parent, genTime);
...
// Events for peers
std::vector<Event *> PeerRcvTxn::callback(Ticks timestamp)
{
    return to->rcvTxn(from, txn, timestamp);
}

std::vector<Event *> PeerGenTxn::callback(Ticks timestamp)
{   
    return peer->genTxn(timestamp);
}

std::vector<Event *> PeerRcvBlk::callback(Ticks tiemstamp)
{
    return to->rcvBlk(from, blk, timestamp);
}

std::vector<Event *> PeerGenBlk::callback(Ticks timestamp)
{
    return peer->genBlk(startTime, timestamp, parent);
}
