...
Event* Link::send(Msg *msg, Ticks sendTime)
{
    ...
    std::cout << src -> ID << " sending to " << tgt -> ID << " will reach at " << aod << " Msg Size: " << msg -> size << std::endl;
...
Event * Peer::start_mining_session(Ticks startTime){
    ...
    //! TODO: Identify parent block to mine on
    BID_t parent = 0;

    //! TODO: Select Txns From Pool
...
std::vector<Event *> Peer::rcvBlk(Peer * src, Blk *blk, Ticks rcvTime)
{
    ...
    if(src != this)
        std::cout << "Blk Recvd by " << ID << " at " << rcvTime << std::endl;
...
std::vector<Event *> Peer::genBlk(Ticks genTime, EID_t eid)
{
    ...
    if(eid != mining_event){
        std::cout << "Invalid Block, mining already started for another block" << std::endl;
        return ret;
    }

    //Create New Block
    Blk *blk = new Blk(ID, TxnForNxtBlk, mining_on, genTime);
    ...
    if(nxtEvent != NULL) ret.push_back(nxtEvent);
    else logerr("Peer::genBlk nxtEvent is NULL");
